			...
			success:function(data){
				...
				for(var i=0;i<knows.length;i++){
					if(knows[i].kSubNum==0){
						$("#kp_div .modal-body").append("<label><input id='kp' name='konwledgePoint' class='kpF"+knows[i].kNum+"' type='checkbox' value="+knows[i].kId+">"+knows[i].kNum+"."+knows[i].kSubNum+"."+knows[i].kTitle+"</label><br>");
					}else{
						$("#kp_div .modal-body").append("<label><input id='kp' name='konwledgePoint' class='kpS"+knows[i].kNum+"' type='checkbox' value="+knows[i].kId+">"+knows[i].kNum+"."+knows[i].kSubNum+"."+knows[i].kTitle+"</label><br>");
					}
				}
				
				if(knows!=""){
					$("#kp_div .modal-footer")
					.append("<button class='btn btn-secondary' onclick='selectAll()'>全选</button>&nbsp;&nbsp;")
					.append("<button class='btn btn-primary' data-dismiss='modal'>确认</button>")
				}
				//全选功能
				clickAll();
			}
	...
	function parseQuestionsData(data,sId,qtId,kps,selectQuestions){
		...
		$.each(questions,function(item){
			...
			$("#questionsTable tbody").append("<tr>" +
				...
				"<button class='btn btn-success btn-sm' data-toggle='modal' data-target='#detailModal'>查</button>" +
				"<button class='btn btn-primary btn-sm' id='questionsAdd_"+questions[item].qId+"' name='questionAdd'>添</button>"+
				"<button class='btn btn-danger btn-sm' id='questionRemove_"+questions[item].qId+"' name='questionRemove'>移</button>"+
				"</tr>");
		})
		
		//分页
		var html="";
		html+="<div class='col-md-4' style='float: left'>共"+page.total+"条数据，当前为第"+page.pageNum+" 页，共"+page.pages+" 页</div>"
			+"<div class='col-md-8' style='float: left'> <nav aria-label='Page navigation' style='float: right;'>"
			+"<ul class='pagination' >  <li ><a id='firstPage' >&laquo;</a></li>"
			+"<li><a id='prePage' aria-label='Previous'><span aria-hidden='true' >&lsaquo;</span>  </a> </li>"
		$.each(page.navigatepageNums,function(index,item){
			html+="<li><a href='#'>"+item+"</a></li>";
		});
			
		html+="<li><a id='nextPage' aria-label='Next'><span aria-hidden='true'>&rsaquo;</span></a></li>"
			+"<li><a id='lastPage'>&raquo;</a></li> </ul></nav></div>";	
	
		$("#questionsTable tfoot tr td").html(html);
		var firstPage=1;
		...
		function midPage(pageNum){
			getQuestionsBySIdAndKnowsAndQt(sId,qtId,kps,selectQuestions,middlePage,pageSize);
		}
...
//全选功能
	function selectAll(){
		var kpNum=$("[id=kp]").length;
		console.log(kpNum);
		var flag=true;
		for(var i=0;i<=kpNum;i++){
		 if($(".kpS"+i).prop("checked")==false){
			 flag=false;
		 }
		}
		for(var i=0;i<=kpNum;i++){
		$(".kpS"+i).prop("checked",!flag);
		}
	}
