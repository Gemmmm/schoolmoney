			...
			success:function(data){
				...
				for(var i in knows){
					if(knows[i].kSubNum==0){
						$("#kp_div .modal-body").append("<label><input name='konwledgePoint' id='kp' class='kpF"+knows[i].kNum+"' type='checkbox' value="+knows[i].kId+">"+knows[i].kNum+"."+knows[i].kSubNum+"."+knows[i].kTitle+"</label><br>");
					}else{
						$("#kp_div .modal-body").append("<label><input name='konwledgePoint'  id='kp' class='kpS"+knows[i].kNum+"' type='checkbox' value="+knows[i].kId+">"+knows[i].kNum+"."+knows[i].kSubNum+"."+knows[i].kTitle+"</label><br>");
					}
				}
				
				if(knows!=""){
					$("#kp_div .modal-footer").append("<button class='btn btn-secondary'  onclick='selectAll()'>全选</button>&nbsp;&nbsp;&nbsp;")
					.append("<button class='btn btn-primary' data-dismiss='modal' id='kp_button_commit'>确认</button>");
				}
				//全选功能
				clickAll();
...
function generateButtonClick(sId){
	...
	$("#paper_table").append("<tr>" +
			...
			"<td>"+scores+"</td>" +
			"</tr>")
	$("#paper_div .modal-footer ").append("<tr><td colspan='5'><button style='margin-left:40%' class='btn btn-success' id='generatePaper'>确认生成</button></td></tr>")
